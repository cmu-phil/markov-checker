...
# A class used to find a good model based on given parameters for ancestral data.
class FindGoodModelAncestral(FindGoodModel):
    """
    A class used to find a good model based on given parameters for ancestral data.
    """

    def __init__(self, location, file=None, num_nodes=10, avg_degree=4, num_latents=0, sample_size=1000):
        super().__init__(location, num_nodes, avg_degree, num_latents, sample_size, 'anclg')
    ...
    def get_model(self, alg, paramValue):
        ...
        if alg == 'fci':
            ...
            _search.use_fisher_z(0.01)
            _search.use_sem_bic(paramValue)
            _search.run_gfci()
        elif alg == 'rfci':
            _search.use_fisher_z(paramValue)
            _search.run_rfci()
    ...
    def cpdag(self, graph):
        return graph.paths().isLegalCpdag()
        # return ts.utils.GraphSearchUtils.isLegalPag(graph).isLegalPag()

    def maps(self):
        dir = f'markov_check_{self.sim_type}'

        penalties = [10.0, 5.0, 4.0, 3, 2.5, 2, 1.75, 1.5, 1.25, 1, 0.75, 0.5]
        alphas = [0.001, 0.01, 0.05, 0.1, 0.2]
            ...
            for avg_degree in range(1, 6 + 1):
                ...
                with (open(f'{self.location}/{dir}/result_{num_nodes}_{avg_degree}.txt', 'w') as file,
                      ...
                      open(f'{self.location}/{dir}/train_{num_nodes}_{avg_degree}.txt',
                           'w') as train_file,
                      open(f'{self.location}/{dir}/test_{num_nodes}_{avg_degree}.txt', 'w') as test_file):
                    find = FindGoodModelAncestral(self.location, file, num_nodes, avg_degree, num_latents, 1000)
                    ...
                    # go through algorithms and parameter choices and save the best lines (print all lines)
                    find.save_best_lines('fci', alphas)
                    find.save_best_lines('gfci', penalties)
                    find.save_best_lines('rfci', alphas)
                    find.save_best_lines('grasp_fci', penalties)
                    find.save_best_lines('bfci', penalties)
