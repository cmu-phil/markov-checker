...
for filename in os.listdir(directory):
    if filename.startswith("result") and filename.endswith(".txt"):  # Process only result files
        ...
        # ep.generate_plot(df, filename, "kldiv", "avgminsd", output_file=output_file, transparency=0.7, USE_KS=True)
        ep.generate_plot(df, filename, "kldiv", "|G|", output_file=output_file)
